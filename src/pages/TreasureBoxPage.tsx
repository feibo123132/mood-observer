...
export const TreasureBoxPage = () => {
  ...
  const controlRecords = useControlStore((state) => state.records);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center relative overflow-hidden">
      ...
      <div className="w-full max-w-md px-6 pb-24 flex-1 overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-4">
          ...
          {/* Tool Card: Control Dichotomy */}
          <motion.div
            layout
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className={`bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden transition-all duration-300 ${
              expandedTool === 'control' ? 'ring-2 ring-purple-100' : ''
            }`}
          >
            {/* Card Header (Clickable to Expand) */}
            <div 
              onClick={() => toggleTool('control')}
              className="p-6 cursor-pointer relative overflow-hidden group"
            >
              <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                 <Scale size={80} className="text-purple-500" />
              </div>
              
              <div className="relative z-10 flex items-start justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-800 mb-1">控制二分法</h3>
                  <p className="text-sm text-slate-500 leading-relaxed">
                    上帝的归上帝，凯撒的归凯撒<br/>
                    <span className="text-xs text-purple-500 mt-2 inline-block">斯多葛学派核心心法 • 强力切断焦虑源 • 能量颗粒归仓</span>
                  </p>
                </div>
                
                <div className="text-slate-300">
                  {expandedTool === 'control' ? <ChevronDown size={24} /> : <ChevronRight size={24} />}
                </div>
              </div>
            </div>
                ...
                >
                  <div className="p-4 space-y-3">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate('/treasure-box/control-dichotomy');
                      }}
                      className="w-full p-4 bg-purple-50 hover:bg-purple-100 rounded-2xl flex items-center justify-between group transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <span className="block font-medium text-slate-800">画线练习</span>
                          <span className="text-xs text-slate-500">将烦恼拆解，让焦虑消失</span>
                        </div>
                      </div>
                      <ChevronRight size={18} className="text-purple-300 group-hover:text-purple-500 transition-colors" />
                    </button>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          <span className="block font-medium text-slate-800">练习记录</span>
                          <span className="text-xs text-slate-500">
                            查看过往的二分法练习 ({controlRecords.length})
                          </span>
                        </div>
                      </div>
                      <ChevronRight size={18} className="text-slate-300 group-hover:text-slate-500 transition-colors" />
