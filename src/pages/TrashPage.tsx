...
import { ArrowLeft, Trash2, RotateCcw, Clock, CheckSquare, Square } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';

export const TrashPage = () => {
  const navigate = useNavigate();
  const { records, restoreRecord, restoreMultipleRecords, permanentDeleteRecord, permanentDeleteMultipleRecords, cleanupTrash } = useMoodStore();
  ...
  const handleBatchPermanentDelete = async () => {
    if (window.confirm(`确定要彻底删除选中的 ${selectedIds.size} 条记录吗？此操作无法撤销。`)) {
  ...
  const handleBatchRestore = async () => {
    if (window.confirm(`确定要恢复选中的 ${selectedIds.size} 条记录吗？`)) {
